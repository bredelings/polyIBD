...
getLociPopAF <- function(snpmatrix_rc_samploci){
  ...
  popAF[1,] <- apply(snpmatrix_rc_samploci[,3:ncol(snpmatrix_rc_samploci)], 1, function(x){(2*(length(which(x==0))) + length(which(x==1)))/(2*length(x))}) # since we know homozygous ref is 0, so this counts as 2 As, and then we count hets and then divide by 2*possible alleles. Doing this roundabout way because we aren't in HWE
